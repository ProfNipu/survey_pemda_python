...
### 6b) Seed Permissions API SIMPEG
Seed permissions untuk modul API SIMPEG:
```bash
...
- **Total Rules**: 3 permission rules

### 6c) Grant Permissions ke Superadmin
Berikan akses penuh ke superadmin:
```bash
docker exec survey_pemda_python_app python manage.py seed_superadmin_full_access
```

### 6d) Akses Menu API SIMPEG
Untuk mengelola data pegawai dari ESIMPEG:
- **Pegawai**: `/api-simpeg/pegawai/`

Atau akses via sidebar: Manajemen Integrasi → ESIMPEG → Pegawai

**Fitur**:
- ✅ List pegawai dari ESIMPEG API
- ✅ Search & pagination
- ✅ Sync data dari ESIMPEG
- ✅ Export data
- ✅ Permission-based access control

### 6e) Import Data Pegawai dari BKPSDM (Optional)
...
## 14) ESIMPEG Token di Session

### Cara Kerja Token ESIMPEG

Saat user login ke Survey Pemda, sistem otomatis:
1. Authenticate user dengan password lokal
2. Call ESIMPEG API untuk mendapatkan token
3. Simpan `esimpeg_access_token` dan `esimpeg_refresh_token` di session
4. Token digunakan untuk akses API SIMPEG (pegawai list, dll)

### User dari Seeder

User yang dibuat via seeder (contoh: `Prakom@admin2025.com`) akan otomatis mendapat token ESIMPEG saat login, asalkan:
- ✅ User ada di ESIMPEG dengan username dan password yang SAMA
- ✅ ESIMPEG API sedang running
- ✅ Password di kedua sistem SAMA
...
#    Seharusnya tampil data pegawai dari ESIMPEG API
```
...
Penyebab: Token tidak ada di session (session lama sebelum fix)
...
Untuk detail teknis tentang ESIMPEG token di session, lihat:
- `docs_dari_sonnet/53_ESIMPEG_TOKEN_FIX.md` - Detail teknis fix
- `docs_dari_sonnet/54_KESIMPULAN_ESIMPEG_TOKEN_SESSION.md` - Kesimpulan lengkap
