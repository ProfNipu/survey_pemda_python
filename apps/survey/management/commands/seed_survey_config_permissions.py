...
class Command(BaseCommand):
    help = 'Seed permissions untuk Survey Configuration'

    def handle(self, *args, **options):
        self.stdout.write('=' * 70)
        self.stdout.write('🌱 Seeding Permissions: Survey Configuration')
        self.stdout.write('=' * 70)
        ...
        # Control: survey_config
        control_config, created = PermissionControl.objects.get_or_create(
            nama_kontrol='survey_config',
            defaults={
                'label_kontrol': 'Konfigurasi Survey',
                'deskripsi_kontrol': 'Kelola konfigurasi form survey dinamis',
            }
        )
        if created:
            self.stdout.write('  ✓ Created control: Konfigurasi Survey')
        ...
        # Create Rules for survey_config
        created_rules = 0
        for func_data in functions_data:
            ...
            if created:
                created_rules += 1
                self.stdout.write(f'    ✓ Created rule: survey.survey_config.{func_data["nama"]}')
